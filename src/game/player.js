...
class Player {
  constructor(camera, scene) {
    ...
    this.speed = 10.0; // Adjusted back to original for consistency
  ...
  // src/game/player.js (update method only)
update(deltaTime, colliders) {
  ...
  newPosition.addScaledVector(moveDirection, 1); // Horizontal movement
  newPosition.y += this.velocity.y * deltaTime; // Vertical movement

  console.log('[Player] Calculated newPosition:', newPosition);
  ...
  // Update position if no collision or after resolution
  if (!collisionDetected || true) { // Always apply resolved position
    this.position.copy(newPosition);
  }

  console.log('[Player] Updated this.position to:', this.position);
  ...
  if (this.mesh) {
    this.mesh.position.copy(this.position);
    this.mesh.position.y -= this.height / 2; // Center at feet
    console.log('[Player] Mesh position:', this.mesh.position);
  }
