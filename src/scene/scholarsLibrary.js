...
class ScholarsLibrary extends THREE.Object3D {
  ...
  async createRoom() {
    ...
    this.addCollisionBox(this.stainedGlass, new THREE.Vector3(6, 6, 0.1));
  }
  ...
  async addFurniture() {
    try {
      ...
      this.addCollisionBox(desk, new THREE.Vector3(8, 5, 5));
    } catch (error) {
    ...
    this.addCollisionBox(this.chandelierGroup, new THREE.Vector3(1.6, 1, 1.6));
  ...
  async addInteractiveObjects() {
    ...
    if (this.skillLevel === 'beginner' || this.skillLevel === 'expert') {
      try {
        ...
        this.add(tome);
        this.addCollisionBox(tome, new THREE.Vector3(1, 0.3, 1.5));
  ...
  update(delta) {
    ...
    this.runeCircles.forEach(rune => {
      if (rune && rune.userData) {
        ...
        if (!this.runeSynced) {
          rune.position.y = 6 + Math.sin(Date.now() * 0.001) * 0.1;
        } // Else, position.y stays at 6 (set in checkRuneSync)
      }
    });
    if (this.magicAura) {
